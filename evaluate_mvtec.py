...
def get_logger(name, save_path=None, level='INFO'):
    ...
    if not save_path is None:
        ...
        fileHandler = logging.FileHandler(os.path.join(save_path, 'MVTec.txt'), encoding='utf-8')
...
def train(item_list):
    ...
    for ji in range(1, 11):
        ...
        k = 14
        memory_features_list = torch.load('E:/model/EmbeddingAD-main/MVTEC2_results/mvtec/14_VISA_memory_features_list4.pth').to(device)
        model.load_state_dict(torch.load('E:/model/EmbeddingAD-main/MVTEC2_results/mvtec/15_model4.pth'))
        auroc_sp_list = []
        ap_sp_list = []
        f1_sp_list = []
        auroc_px_list = []
