...
def get_logger(name, save_path=None, level='INFO'):
    ...
    if not save_path is None:
        ...
        fileHandler = logging.FileHandler(os.path.join(save_path, 'finalVisa.txt'), encoding='utf-8')
...
def train(item_list):
    ...
    for ji in range(4, 5):
        if ji == 1:
            ...
            featuresampler = get_sampler(device, name="approx_greedy_coreset", percentage=784)
        featuresampler = get_sampler(device, name="approx_greedy_coreset", percentage=784)
        image_sampler = get_image_sampler(device, name="approx_greedy_coreset", percentage=0.15)
        lamda = 1 - 8 * 0.05
        beta = 0 + 0.025 * ji
        ...
        for train_loader in train_dataloader_list:
            ...
            epochs = tqdm(range(7), desc="Training Epochs", total=7)
            if k > 0:
                ...
                for _, (img, label) in progress_bar:    
                    ...
                    current_batch_features2 = _detach(en_list.reshape(-1, 784*768).contiguous())
                ...
                current_task_en_list = torch.from_numpy(current_batch_array2_all).view(-1, 784, 768).contiguous().cpu()
                ...
                for epoch in epochs:
                    ...
                    for _, en in progress_bar:
                        ...
                        # with torch.no_grad():   
                        #     for model_convex, modelbottleneck, KDbottleneck in zip(convex_bottleneck.parameters(), model.bottleneck.parameters(), KD_bottleneck.parameters()):
                        #         model_convex.data = modelbottleneck.data * lamda + KDbottleneck.data * (1.0 - lamda)
                        #     for model_convex, modeldecoder, KDdecoder in zip(convex_decoder.parameters(), model.decoder.parameters(), KD_decoder.parameters()):
                        #         model_convex.data = modeldecoder.data * lamda + KDdecoder.data * (1.0 - lamda)
                        # with torch.no_grad():
                        #     pred = getKDprediction(en, convex_bottleneck, convex_decoder).detach()
                        # output = de[0].permute(0, 2, 3, 1).reshape([de[0].shape[0], -1, de[0].shape[1]]) # torch.Size([16, 784, 768])
                        # min_loss = global_cosine(pred, output)  # torch.Size([16, 768, 28, 28])
                        # else:
                        #     min_loss = torch.tensor(0.0).to(device)
            ...
            memory_features = memory_features.view(-1, 784, 768).contiguous()  # 将内存特征转换为PyTorch张量，并移动到指定的设备上。
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='./VISA2__results')
