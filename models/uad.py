...
class ViTill(nn.Module):
    def __init__(
        ...
        self.Projector = nn.Sequential(OrderedDict([
            ('Proj_Lin_1', nn.Linear(in_features=768,out_features=768)),
            ('Proj_ReLU_1', nn.GELU()),
            ('Proj_Lin_2', nn.Linear(in_features=768, out_features=768)),
        ]))
    ...
    def forward(self, x, train=0):
        if train == 0:
            x = self.encoder.prepare_tokens(x)#torch.Size([16, 789, 768])
            en_list = []
            for i, blk in enumerate(self.encoder.blocks):
                if i <= self.target_layers[-1]:
                    if i in self.encoder_require_grad_layer:
                        ...
                        with torch.no_grad():
                            x = blk(x)#torch.Size([16, 789, 768])
                else:
                    continue
                if i in self.target_layers:
                    en_list.append(x)
            side = int(math.sqrt(en_list[0].shape[1] - 1 - self.encoder.num_register_tokens))#28

            if self.remove_class_token:
                en_list = [e[:, 1 + self.encoder.num_register_tokens:, :] for e in en_list]

            x = self.fuse_feature(en_list)#torch.Size([16, 789, 768])，传给MLP
            for i, blk in enumerate(self.bottleneck):#MLP
                x = blk(x)
            ...
            en = [self.fuse_feature([en_list[idx] for idx in idxs]) for idxs in self.fuse_layer_encoder]#将1，2，3，4；5，6，7，8层的特征均值融合
            ...
            en = [e.permute(0, 2, 1).reshape([x.shape[0], -1, side, side]).contiguous() for e in en]#torch.Size([2, 16, 768, 28, 28])
            de = [d.permute(0, 2, 1).reshape([x.shape[0], -1, side, side]).contiguous() for d in de]#torch.Size([2, 16, 768, 28, 28])
            return en, de, self.fuse_feature(en_list)
        elif train == 1:
            ...
            de = [self.fuse_feature([de_list[idx] for idx in idxs]) for idxs in self.fuse_layer_decoder]#将1，2，3，4；5，6，7，8层的特征均值融合

            if not self.remove_class_token:  # class tokens have not been removed above
                en = [e[:, 1 + self.encoder.num_register_tokens:, :] for e in en]
                de = [d[:, 1 + self.encoder.num_register_tokens:, :] for d in de]

            en = [e.permute(0, 2, 1).reshape([x.shape[0], -1, 28, 28]).contiguous() for e in en]#torch.Size([2, 16, 768, 28, 28])
            de = [d.permute(0, 2, 1).reshape([x.shape[0], -1, 28, 28]).contiguous() for d in de]#torch.Size([2, 16, 768, 28, 28])
            return en, de
        elif train == 3:
            en = [x]
            # if self.remove_class_token:
