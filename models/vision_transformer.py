...
class SepConv(nn.Module):
    ...
    def forward(self, x):
        b, hxw, c = x.shape
        h = int(math.sqrt(hxw))
        #x = self.pwconv1(x)#torch.Size([16, 784, 1536])
        x = self.act1(x)#torch.Size([16, 784, 1536])
        x = x.permute(0, 2, 1).reshape(b, -1, h, h)#torch.Size([16, 1536, 28, 28])
        x = self.dwconv(x)#torch.Size([16, 1536, 28, 28])
        x = x.permute(0, 2, 3, 1).reshape(b, hxw, -1)#torch.Size([16, 784, 1536])
        x = self.act2(x)#torch.Size([16, 784, 1536])
        #x = self.pwconv2(x)#torch.Size([16, 784, 768])
        return x
