...
def global_cosine_hm_percent(a, b, p=0.9, factor=0.):
    ...
    for item in range(len(a)):
        ...
        b_ = b[item]
        with torch.no_grad():
            point_dist = 1 - cos_loss(a_, b_).unsqueeze(1)# 每个 token 的重建误差（越大表示越异常/难重建）torch.Size([16, 1, 28, 28])
        # mean_dist = point_dist.mean()
        # std_dist = point_dist.reshape(-1).std()
...
import torch
def replace_with_closest_memory_feature(feature_list, memory_tensor):
    """
    将列表中的每个特征点替换为记忆库中欧式距离最小的记忆特征点。

    参数:
        feature_list (list[torch.Tensor]): 列表中存放三维张量，每个张量形状为 [batchsize, num_features, feature_dim]。
        memory_tensor (torch.Tensor): 三维记忆库张量，形状为 [memory_batchsize, memory_num_features, feature_dim]。

    返回:
        list[torch.Tensor]: 替换后的特征列表。
    """
    # 将记忆库张量的前两个维度堆叠成一个维度，形状变为 [memory_batchsize * memory_num_features, feature_dim]
    memory_tensor_flat = memory_tensor.view(-1, memory_tensor.size(-1)).to(device)
    ...
    for features in feature_list:
        # 获取当前特征张量的形状
        batchsize, num_features, feature_dim = features.shape

        # 展平当前特征张量为 [batchsize * num_features, feature_dim]
        features_flat = features.view(-1, feature_dim)

        # 计算欧式距离
        distances = torch.cdist(features_flat, memory_tensor_flat, p=2)  # [batchsize * num_features, memory_batchsize * memory_num_features]

        # 找到每个特征点欧式距离最小的记忆特征点的索引
        closest_indices = torch.argmin(distances, dim=1)  # [batchsize * num_features]

        # 根据索引从记忆库中提取最相似的记忆特征点
        closest_memory_features = memory_tensor_flat[closest_indices]  # [batchsize * num_features, feature_dim]

        # 将替换后的特征恢复为原始形状 [batchsize, num_features, feature_dim]
        replaced_features_tensor = closest_memory_features.view(batchsize, num_features, feature_dim)

        # 添加到结果列表
        replaced_features.append(replaced_features_tensor)
...
def cal_anomaly_maps(fs_list, ft_list, out_size=224):
    if not isinstance(out_size, tuple):
        out_size = (out_size, out_size)#输出的图像大小

    a_map_list = []
    for i in range(len(ft_list)):
        fs = fs_list[i]#[16, 768, 28, 28]
        ft = ft_list[i]
        a_map = 1 - F.cosine_similarity(fs, ft)#[16, 28, 28]
        a_map = torch.unsqueeze(a_map, dim=1)#[16, 1, 28, 28]
        a_map = F.interpolate(a_map, size=out_size, mode='bilinear', align_corners=True)#(16, 1, 392, 392)   
        a_map_list.append(a_map)
    anomaly_map = torch.cat(a_map_list, dim=1).mean(dim=1, keepdim=True)#torch.Size([16, 1, 392, 392])
    return anomaly_map, a_map_list
...
def min_max_norm(image, a_min=0, a_max=1):
    return (image - a_min) / (a_max - a_min)
...
def evaluation(model, dataloader, device, _class_=None, calc_pro=True, norm_factor=None, feature_used='all',
    ...
    with torch.no_grad():
        for img, gt, label, _ in tqdm(dataloader, desc="Evaluating"):
            img = img.to(device)
...
def evaluation_batch(model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    ...
    with torch.no_grad():
        for img, gt, label, img_path in tqdm(dataloader, desc="Evaluating Batch"):
            img = img.to(device)
            # starter.record()
            output = model(img)
            # ender.record()
            ...
            en, de = output[0], output[1]#torch.Size([1, 16, 768, 28, 28])

            anomaly_map, _ = cal_anomaly_maps(en, de, img.shape[-1])#torch.Size([16, 1, 392, 392])
            # anomaly_map = anomaly_map - anomaly_map.mean(dim=[1, 2, 3]).view(-1, 1, 1, 1)

            if resize_mask is not None:
                anomaly_map = F.interpolate(anomaly_map, size=resize_mask, mode='bilinear', align_corners=False)#torch.Size([16, 1, 256, 256])
                gt = F.interpolate(gt, size=resize_mask, mode='nearest')

            anomaly_map = gaussian_kernel(anomaly_map)#torch.Size([16, 1, 256, 256])
...
import time
def evaluation_previous_batch(model, dataloader, device, memory_tensor, _class_=None, max_ratio=0, resize_mask=None):
    model.eval()
    gt_list_px = []
    pr_list_px = []
    gt_list_sp = []
    ...
    with torch.no_grad():
        ...
        for img, gt, label, img_path in tqdm(dataloader, desc="Evaluating Previous Batch"):
            img = img.to(device)
            # starter.record()
            en = model(img, train=1)
            # ender.record()
            en1 = replace_with_closest_memory_feature([en], memory_tensor)
            en1, de = model(en1[0], train=2)
            en = en.permute(0, 2,  1).view(en.shape[0], en.shape[2], 28, 28)  # [B, H, W, C]
            # torch.cuda.synchronize()
            # curr_time = starter.elapsed_time(ender)
...
def evaluation_batch_loco(model, dataloader, device, _class_=None, max_ratio=0):
    ...
    with torch.no_grad():
        for img, gt, label, path, defect_type, size in tqdm(dataloader, desc="Evaluating Batch Loco"):
            img = img.to(device)
...
def evaluation_uniad(model, dataloader, device, _class_=None, reg_calib=False, max_ratio=0):
    ...
    with torch.no_grad():
        for img, gt, label, _ in tqdm(dataloader, desc="Evaluating UniAD"):
            img = img.to(device)
            if reg_calib:
...
def visualize(model, dataloader, device, _class_='None', save_dir='None', save_name='save'):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for img, gt, label, img_path in tqdm(dataloader, desc="Visualizing"):
            img = img.to(device)
            output = model(img)
            en, de = output[0], output[1]
            anomaly_map, _ = cal_anomaly_maps(en, de, img.shape[-1])
            anomaly_map = gaussian_kernel(anomaly_map)
            min_score = min(min_score, anomaly_map.min().item())
            max_score = max(max_score, anomaly_map.max().item())
            for i in range(anomaly_map.shape[0]):
                anomaly_map_list.append(anomaly_map[i])
                img_list.append(img[i])
                gt_list.append(gt[i])
                img_path_list.append(img_path[i])
        for i in range(0, len(anomaly_map_list), 1):
            heatmap = min_max_norm(anomaly_map_list[i][0].cpu().numpy(), a_min=min_score, a_max=max_score)
            heatmap = cvt2heatmap(heatmap * 255)
            im = img_list[i].permute(1, 2, 0).cpu().numpy()
            im = im * np.array([0.229, 0.224, 0.225]) + np.array([0.485, 0.456, 0.406])
            im = (im * 255).astype('uint8')
            im = im[:, :, ::-1]
            hm_on_img = show_cam_on_image(im, heatmap)
            mask = (gt_list[i][0].numpy() * 255).astype('uint8')
            save_dir_class = os.path.join(save_dir, str(_class_))
            if not os.path.exists(save_dir_class):
                os.mkdir(save_dir_class)
            name = img_path_list[i].split('/')[-2] + '_' + img_path_list[i].split('/')[-1].replace('.png', '')
            class_dir = img_path_list[i].split('\\')[-2]
            name = img_path_list[i].split('\\')[-1].replace('.png', '')
            if not os.path.exists(save_dir_class + '/' + class_dir):
                os.mkdir(save_dir_class + '/' + class_dir)
            cv2.imwrite(save_dir_class + '/' + class_dir + '/' + name + '_img.png', im)
            cv2.imwrite(save_dir_class + '/' + class_dir + '/' + name + '_cam.png', hm_on_img)
            cv2.imwrite(save_dir_class + '/' + class_dir + '/' + name + '_gt.png', mask)

    return

def visualize_previous(model, dataloader, memory_tensor, device, _class_='None', save_dir='None', save_name='save'):
    model.eval()
    save_dir = os.path.join(save_dir, save_name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    max_score = -1000
    with torch.no_grad():
        for img, gt, label, img_path in tqdm(dataloader, desc="Visualizing Previous"):
            img = img.to(device)
            en = model(img, train=1)
            # ender.record()
            en1 = replace_with_closest_memory_feature([en], memory_tensor)
...
def calculate_means(array1: np.ndarray, array2: np.ndarray):
    """
    接收两个形状为 (15, 2) 的数组，计算第一个数组与第二个数组逐元素相减后，
    分别求第二维度中两维度的均值。

    Args:
        array1 (np.ndarray): 第一个形状为 (15, 2) 的数组。
        array2 (np.ndarray): 第二个形状为 (15, 2) 的数组。

    Returns:
        tuple: 包含两维度均值的元组 (mean_dim1, mean_dim2)。
    """
    # if array1.shape != (15, 2) or array2.shape != (15, 2):
    #     raise ValueError("输入数组的形状必须为 (15, 2)")

    diff = array1 - array2
    mean_dim1 = np.mean(diff[:, 0])
    mean_dim2 = np.mean(diff[:, 1])
...
def visualize_feature_maps(en):
    """
    对特征图列表进行灰度可视化。

    Args:
        en (list): 包含特征图的列表，每个特征图形状为 [1, C, H, W]。
    """
    for idx, feature_map in enumerate(en):
        if len(feature_map.shape) != 4 or feature_map.shape[0] != 1:
            raise ValueError("每个特征图必须是形状为 [1, C, H, W] 的张量。")

        # 将特征图转换为灰度图
        feature_map = feature_map.squeeze(0)  # 移除批次维度，形状变为 [C, H, W]
        grayscale_map = np.mean(feature_map, axis=0)  # 对通道维度求均值，形状变为 [H, W]

        # 可视化灰度图
        plt.figure(figsize=(6, 6))
        plt.imshow(grayscale_map, cmap='gray')
        plt.title(f'Feature Map {idx + 1}', fontsize=14)
        plt.axis('off')
        plt.colorbar()
        plt.show()
